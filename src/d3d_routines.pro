...
PRO d3d_routines,inputs,grid,$ 			;;INPUT: INPUTS AND INTERPOLATING GRID
    ...
    if where('use_transp' eq inVars) eq -1 then begin
        inputs = create_struct(inputs,'use_transp','') ;;DEFAULT TO GAPROFILES
    endif
	...
	;;GET PROFILES
    if inputs.use_transp ne '' then begin
        profiles=create_transp_profiles(inputs,grid,equil)
    endif else begin
	    profiles=d3d_profiles(inputs,grid,equil)
    endelse
